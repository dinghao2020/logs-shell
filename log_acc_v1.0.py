...
def account_all(date1 ,date2):
    ...
    frequency_sql = subprocess.check_output('''awk -F']' '{print $2}' %s|awk -F'[' '{++S[$2]} END {for (a in S) {print S[a], a}}'|awk 'date0=strftime("%%Y%%m%%d",systime()-'%s'*24*3600) {print "insert %s (date, count,attribute, type) value ("date0" ,"$1" ,"$2", 1);"}' ''' % (logs(date1), n, frequency_table), shell=True).decode("utf-8").strip("\n")
...
if dd == "day":
    ...
    n = subprocess.check_output('date +%e', shell=True).decode("utf-8").strip("\n")
    insert_date = subprocess.check_output('date -d "%s days ago" +%%Y%%m%%d' % n, shell=True).decode("utf-8").strip("\n")
