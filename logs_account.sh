...
MYSQL1="mysql -h192.168.3.102 -udh -pgitLgit dinghaot1 --default-character-set=utf8 -A -N"
...
case $1 in
day)
        date1=$(date -d '1 days ago' +%Y%m%d).log
        date2=$(date -d '2 days ago' +%Y%m%d).log
        n=1
        ...
        ;;
week)
        date1=$(for i in {1..7};do eval d$i=$(/bin/date -d "$i days ago" +%Y%m%d) && echo $[d$i].log;done)
        date2=$(for i in {8..14};do eval d$i=$(/bin/date -d "$i days ago" +%Y%m%d) && echo $[d$i].log;done)
        n=7
        ...
        ;;
month)
        date1=$(date +%Y%m --date="-1 month")*.log
        date2=$(date +%Y%m --date="-2 month")*.log
        n=$(date +%e)
